...
tensor_list RasterizeGaussiansCPUEnhanced::forward(
    AutogradContext *ctx, torch::Tensor xys, torch::Tensor radii,
    torch::Tensor conics, torch::Tensor colors, torch::Tensor opacity,
    torch::Tensor cov2d, torch::Tensor camDepths, int imgHeight, int imgWidth,
  ...
  torch::Tensor depthMap = torch::zeros(
      {height, width},
      torch::TensorOptions().dtype(torch::kFloat32).device(device));
  torch::Tensor alphaMap = torch::zeros(
      {height, width},
      torch::TensorOptions().dtype(torch::kFloat32).device(device));
  torch::Tensor px2gidTensor =
      torch::full({height, width, max_gaussians_per_pixel}, -1,
  ...
  for (const size_t &gaussianIdx : gIndices) {
    ...
    const int maxX =
        static_cast<int>(std::min(static_cast<float>(width), centerX + sqCovX));
    const int minY = static_cast<int>(std::max(0.0f, centerY - sqCovY));
    const int maxY = static_cast<int>(
        std::min(static_cast<float>(height), centerY + sqCovY));
    ...
    for (int y = minY; y < maxY; y++) {
      for (int x = minX; x < maxX; x++) {
        ...
        const float power =
            -0.5f *
            (conic00 * dx * dx + 2.0f * conic01 * dx * dy + conic11 * dy * dy);
        ...
        if (px2gid_count[pixIdx] < max_gaussians_per_pixel) {
          pPx2gid[pixIdx * max_gaussians_per_pixel + px2gid_count[pixIdx]] =
              static_cast<int32_t>(gaussianIdx);
          px2gid_count[pixIdx]++;
        }
...
tensor_list RasterizeGaussiansCPUEnhanced::backward(AutogradContext *ctx,
...
tensor_list RasterizeGaussiansEnhanced::forward(
    AutogradContext *ctx, torch::Tensor xys, torch::Tensor depths,
    torch::Tensor radii, torch::Tensor conics, torch::Tensor numTilesHit,
    torch::Tensor colors, torch::Tensor opacity, int imgHeight, int imgWidth,
    torch::Tensor background) {
  ...
  // Create placeholder px2gid tensor (GPU version doesn't implement full px2gid
  // yet)
  const int max_gaussians_per_pixel = 32;
  torch::Tensor px2gidTensor = torch::full(
      {imgHeight, imgWidth, max_gaussians_per_pixel}, -1,
...
tensor_list RasterizeGaussiansEnhanced::backward(AutogradContext *ctx,
                                                 tensor_list grad_outputs) {
  // Extract saved data
  torch::Tensor xys = ctx->saved_data["xys"].toTensor();
...
EnhancedRenderOutput render_gaussians_enhanced(
    torch::Tensor means, torch::Tensor scales, torch::Tensor quats,
    torch::Tensor features_dc, torch::Tensor features_rest,
    torch::Tensor opacities, torch::Tensor viewMat, torch::Tensor projMat,
    float fx, float fy, float cx, float cy, int height, int width,
    int sh_degree, torch::Tensor background, const torch::Device &device) {

  // Combine spherical harmonics features
  torch::Tensor colors = torch::cat(
      {features_dc.index({torch::indexing::Slice(), torch::indexing::None,
  ...
  if (device.is_cpu()) {
    throw std::runtime_error(
        "CPU rendering is not supported. Please use CUDA device.");
  } else {
#if defined(USE_HIP) || defined(USE_CUDA)
  ...
  torch::Tensor viewDirs =
      means.detach() -
      viewMat
          .index({torch::indexing::Slice(torch::indexing::None, 3),
...
#if defined(USE_HIP) || defined(USE_CUDA)
  result = RasterizeGaussiansEnhanced::apply(
      xys, depths, radii, conics, numTilesHit, rgbs, torch::sigmoid(opacities),
      height, width, background);
#else
  throw std::runtime_error(
      "CUDA/HIP support not compiled. Please rebuild with GPU support.");
#endif

  if (result.size() != 4) {
    throw std::runtime_error("Enhanced rasterizer should return 4 tensors "
                             "(RGB, depth, alpha, px2gid)");
  }

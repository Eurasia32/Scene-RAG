...
torch::Tensor RenderModel::render(const torch::Tensor &viewMat,
                                  const torch::Tensor &projMat, float fx,
                                  float fy, float cx, float cy, int height,
                                  int width, const torch::Tensor &background) {
  ...
  auto rasterResults = rasterize_forward_tensor(
      ...
      std::make_tuple(width, height, 1),
      gaussianIdsSorted, tileBins, xys, conics, rgbs,
      torch::sigmoid(opacities), background);
  
  rgb = std::get<0>(rasterResults);
#else
  throw std::runtime_error(
